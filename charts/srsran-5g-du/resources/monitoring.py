...
def main():
    ...
    pstate_info = print_pstate_info() # Print global pstate info once
    ...
    # Define Header - Added Governor, EPB, MinP%, MaxP%. Removed Throttle Counts. Adjusted widths. Using TAB delimiter.
    #                Core CPU  ActMHz   Avg_MHz   Busy%   Bzy_MHz   TSC_MHz         IRQ    C1%   C1E%    C6% CoreTmp CoreThr  PkgTmp MinP% MaxP% Governor   EPB PkgWatt RAMWatt
    ...
    header_str = header_fmt.format(
        ...
        "C1%", "C1E%", "C6%",
        "CoreTmp", "CoreThr", "PkgTmp", "MinP%", "MaxP%", # Added PState %
        "Governor", "EPB",
        "PkgWatt", "RAMWatt"
    )
    ...
    while True:
        try:
            # Pass pstate_info which contains paths for min/max perf pct
            cpu_data, pkg_data = get_all_counters(target_cpus, topology, tjmax, rapl_domains_info, cpuidle_state_info, pstate_info)

            if prev_cpu_data is not None:
                delta_cpu_data = {}
                delta_pkg_data = {}
                valid_delta = True

                for cpu_id in target_cpus:
                    delta = cpu_data[cpu_id].delta(prev_cpu_data[cpu_id])
                    if delta is None: valid_delta = False; break
                    delta_cpu_data[cpu_id] = delta
                if valid_delta:
                     for pkg_id in pkg_data:
                          if pkg_id in prev_pkg_data:
                                delta = pkg_data[pkg_id].delta(prev_pkg_data[pkg_id])
                                if delta is None: valid_delta = False; break
                ...
                if valid_delta:
                    ...
                    for cpu_id in target_cpus:
                        ...
                        pkg_watt = (d_pkg.energy_pkg_uj / 1_000_000) / interval_sec if d_pkg and interval_sec > 0 else 0.0
                        ram_watt = (d_pkg.energy_dram_uj / 1_000_000) / interval_sec if d_pkg and interval_sec > 0 else 0.0
                        ...
                        print(header_fmt.format(
                            ...
                            str(delta.governor) if delta.governor else "",
